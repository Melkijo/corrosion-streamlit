...
st.title("The Implementation of Machine Learning for Corrosion Detection")
...
class getGrayRumatrix:
    ...
    def getGrayLevelRumatrix(self, array, theta):
        ...
        deg90 = [
            val.tolist() for sublist in np.split(np.transpose(P), y) for val in sublist
        ]  # 90deg
        diags = [
            P[::-1, :].diagonal(i) for i in range(-P.shape[0] + 1, P.shape[1])
        ]  # 45deg
        ...
        for angle in theta:
            for splitvec in range(0, len(eval(angle))):
                ...
                for ansIndex in range(0, len(answer)):
                    glrlm[
                        int(answer[ansIndex][0] - min_pixels),
                        int(answer[ansIndex][1] - 1),
                        theta.index(angle),
                    ] += 1
        return glrlm
    ...
    def getShortRunEmphasis(self, rlmatrix):
        ...
        numerator = np.apply_over_axes(
            np.sum, self.apply_over_degree(np.divide, rlmatrix, (J * J)), axes=(0, 1)
        )[0, 0]
    ...
    def getLongRunEmphasis(self, rlmatrix):
        I, J = self.calcuteIJ(rlmatrix)
        numerator = np.apply_over_axes(
            np.sum, self.apply_over_degree(np.multiply, rlmatrix, (J * J)), axes=(0, 1)
        )[0, 0]
        S = self.calcuteS(rlmatrix)
        return numerator / S
    ...
    def getLowGrayLevelRunEmphasis(self, rlmatrix):
        I, J = self.calcuteIJ(rlmatrix)
        numerator = np.apply_over_axes(
            np.sum, self.apply_over_degree(np.divide, rlmatrix, (I * I)), axes=(0, 1)
        )[0, 0]
        S = self.calcuteS(rlmatrix)
        return numerator / S
    ...
    def getHighGrayLevelRunEmphais(self, rlmatrix):
        I, J = self.calcuteIJ(rlmatrix)
        numerator = np.apply_over_axes(
            np.sum, self.apply_over_degree(np.multiply, rlmatrix, (I * I)), axes=(0, 1)
        )[0, 0]
        S = self.calcuteS(rlmatrix)
        return numerator / S
    ...
    def getShortRunHighGrayLevelEmphasis(self, rlmatrix):
        ...
        numerator = np.apply_over_axes(
            np.sum, self.apply_over_degree(np.divide, temp, (J * J)), axes=(0, 1)
        )[0, 0]
        S = self.calcuteS(rlmatrix)
        return numerator / S
...
def glrlm_prediction(image_path):
    ...
    # Resize Image
    image_new = ImageOps.fit(
        image, target_size, method=0, bleed=0.0, centering=(0.5, 0.5)
    )
    ...
    glrlm_result = glr_matrix_calculator.getGrayLevelRumatrix(
        glr_matrix_calculator.data, theta
    )
    ...
    selected_features = {
        ...
        if key in desired_features
    }

    # Transform the dictionary to a 1D NumPy array
    transformed_features_array = np.array(list(selected_features.values())).reshape(
        1, -1
    )
...
def glcm_prediction(image_path):
    ...
    def compute_glcm_features(image_path):
        ...
        matrix_cooccurrence = graycomatrix(
            inds,
            [1],
...
image_path = st.file_uploader("Upload file")
